...
def create_data_test(sampler,type_transfo=None,):
    ...
    elif type_transfo=="sampling_ssh_sst15":
        
        sst_down = sst[:,0,::15,::15]
        lon_down = lon[::15,::15]
        lat_down = lat[::15,::15]
        sst = downscalling_and_interpolation(sst_down,sst,lat_down,lon_down)
...
for sampler in list_sampler:
    (supermodel_surface,bhatta_surface,std) = score_and_supermodel(sampler,type_transfo='pooling_ssh_sst10',model="sst2_ssh2")
    bl_list_pool.append(bhatta_surface)
    std_list_pool.append(std)
    
    (supermodel_surface,bhatta_surface,std) = score_and_supermodel(sampler,type_transfo='pooling_ssh_sst10',model="sst24_ssh80")
    bl_list_pool_80.append(bhatta_surface)
    std_list_pool_80.append(std)
...
for sampler in list_sampler:
    (supermodel_surface,bhatta_surface,std) = score_and_supermodel(sampler,type_transfo='sampling_ssh_sst15',model="sst2_ssh2")
    bl_list.append(bhatta_surface)
    std_list.append(std)
    
    (supermodel_surface,bhatta_surface,std) = score_and_supermodel(sampler,type_transfo='sampling_ssh_sst15',model="sst24_ssh80")
    bl_list_80.append(bhatta_surface)
    std_list_80.append(std)


nc_file = nc.Dataset("score_sensibility_ssh_res.nc",'w')

#Dimensions used
nc_file.createDimension('reso', len(list_sampler))
nc_file.createDimension('time', np.array(bl_list_80).shape[1])

nc_file.createVariable('bl_list_pool', 'f4', ('reso','time'))
nc_file.createVariable('bl_list_pool_80', 'f4', ('reso','time'))
...
nc_file.variables['bl_list_pool'][:] = bl_list_pool
nc_file.variables['bl_list_pool_80'][:] = bl_list_pool_80
