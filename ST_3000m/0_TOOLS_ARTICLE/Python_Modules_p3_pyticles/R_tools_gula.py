...
def rot2uv(u,v,pm,pn,verbo=False,potential=False):
    ...
    # compute rot
    div = np.zeros(pm.shape)
    div[:,1:-1] = div[:,1:-1] - rho2v(u2rho(diffy(u,rho2u(pn))))
    div[1:-1,:] = div[1:-1,:] + rho2u(v2rho(diffx(v,rho2v(pm))))
    div[np.isnan(div)] =0
...
def rot2uv_variable(u,v,pm,pn,verbo=False,fast=False,potential=False):
    ...
    # compute rot
    div = np.zeros(pm.shape)
    div[:,1:-1] = div[:,1:-1] - diffy(rho2v(u2rho(u)),rho2v(pn))
    div[1:-1,:] = div[1:-1,:] + diffx(rho2u(v2rho(v)),rho2u(pm))
    div[np.isnan(div)] =0
...
def rot2streams(u,v,pm,pn,verbo=False,variable=True,fast=True):
