...
def vinterp(var, depths, z_r, z_w=None, mask=None,imin=0,jmin=0,kmin=1, floattype=np.float64,interp_sfc=1,interp_bot=0,below=None,verbo=False,**kwargs):
...
def vinterp_2d(var, depths, z_r, z_w=None, mask=None,imin=0,jmin=0,kmin=1, floattype=np.float64,below=None,verbo=False,**kwargs):
    ...
    if mask==None:  mask = np.ones((z_r.shape[0]), order='F', dtype=floattype); mask[z_r[:,-1]==0] = 0
    ...
    if np.rank(depths)==1: newz = np.asfortranarray(np.zeros((z_r.shape[0],len(depths))) + depths, dtype=floattype)
    else: newz = depths
...
PV is computed on horizontal psi-grid and vertical rho-grid (specified by z_r)
