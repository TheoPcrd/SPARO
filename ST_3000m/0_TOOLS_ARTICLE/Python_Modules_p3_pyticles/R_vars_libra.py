...
###################################################################################
    ...
    def __init__(self,varname,simul,n2max=10000,method='new',verbo=False,**kwargs):
        ...
        ####################################################################
        if (self.name in list(ncfile.variables.keys())) and (self.name not in ['w','rho']):
        ...
        ####################################################################
            ...
            for i,j in product(list(range(nchunk)),list(range(nchunk))):
                ...
                elif (len(depths)==1) and (self.name in ['vrt','absvrt']):

                    
                    chunk = self.get_sig(ncfile,simul,coord=[ny1i,ny2i,nx1i,nx2i],subcoord=[ny1i-ny1,ny2i-ny1,nx1i-nx1,nx2i-nx1])
