...
def get_depths(simul,**kwargs):
    ...
    if 'coord' in  kwargs: 
        ...
        #print(ny1,ny2,nx1,nx2,ny1i,ny2i,nx1i,nx2i)
...
def vinterp(var, depths, z_r, z_w=None, mask=None,imin=0,jmin=0,kmin=1, floattype=np.float64,interp_sfc=1,interp_bot=0,below=None,verbo=False,bounded=False,**kwargs):
    ...
    if np.ndim(depths)==1: newz = np.asfortranarray(np.zeros((var.shape[0],var.shape[1],len(depths))) + depths, dtype=floattype)
    else: newz = depths
...
def rotuv(simul,u,v,psi=True,**kwargs):
    ...
    if isinstance(simul, float) or isinstance(simul,np.ndarray):
        angle = simul 
    else:
        if 'coord' in  kwargs: 
...
def find_nearest_points(x,y,lon,lat):

    # zip them together
    model_grid = list( zip(np.ravel(x), np.ravel(y)) )
    
    #target point location :
    target_pts = [lon,lat]
    distance, index = sp.KDTree(model_grid).query(target_pts)
    i,j = index//x.shape[1], index%x.shape[1]
...
#Absolute vorticity,  [f + (dv/dx - du/dy)]
    ...
    vrt = get_vrt(u,v,pm,pn,z_r,z_w,mask)
...
#######################################################
#Compute okubo-Weiss of a 3-D field on rho grid
#######################################################
def get_ow(u,v,z_r,z_w,pm,pn,mask=None):

    if len(u.shape)==3:
        #dudy and dvdx on psi grid
        dvdx = diffxi(v,rho2v(pm),rho2v(z_r),rho2v(z_w),mask)
        dudy = diffeta(u,rho2u(pn),rho2u(z_r),rho2u(z_w),mask)
        dvdy = diffeta(v,rho2v(pn),rho2v(z_r),rho2v(z_w),mask)
        dudx = diffxi(u,rho2u(pm),rho2u(z_r),rho2u(z_w),mask)
    else:
        dvdx = diffx(v,rho2v(pm))
        dudy = diffy(u,rho2u(pn))
    ...
    if len(u.shape)==3:
        dvdy = np.pad(dvdy,pad_width = ((0, 0), (1, 1), (0, 0)),mode='edge')
        dudx = np.pad(dudx,pad_width = ((1, 1), (0, 0), (0, 0)),mode='edge')
    else:
    ...
    # ow = np.abs(strain(u,v,pm,pn))**2 -  np.abs(psi2rho(vort(u,v,pm,pn)))**2

    ow = (dudx-dvdy)**2 + (dudy+dvdx)**2 - (dvdx-dudy)**2
