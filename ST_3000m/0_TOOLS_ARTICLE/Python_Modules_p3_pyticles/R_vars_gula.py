...
###################################################################################
    ...
    def __init__(self,varname,simul,n2max=50000,method='new',debug=False,**kwargs):
        ...
        ####################################################################
            ...
            if (self.kmin>=0) and (np.min(depths)>0) and (len(depths)>1):
            ...
            for i,j in product(list(range(nchunk)),list(range(nchunk))):
                ...
                if (self.name[:4] in ['int_']):
                    #Variables with names beginning by int_ are variables vertically integrated between 2 depths (so 2D variables but len(depths)!=1)
                ...
                elif (len(depths)==1) and (np.min(depths)==0) and (self.name in ['vrt','absvrt','ow']):
                    ...
                    chunk = tools.vinterp(self.get_sig(ncfile,simul,depths=simul.coordmax[4],coord=[ny1i,ny2i,nx1i,nx2i],subcoord=[ny1i-ny1,ny2i-ny1,nx1i-nx1,nx2i-nx1]),\
                            depths,z_r,z_w,imin=self.imin,jmin=self.jmin,kmin=self.kmin,floattype = simul.floattype,debug=debug)
        ...
        ################################################
            ...
            for i in range(rho.shape[0]):
                for j in range(rho.shape[1]):
                    if np.where((rho[i,j,:] - rho[i,j,-1])>0.03)[0].size == 0:
                         var[i,j] = topo[i,j]
                    else:
...
#######################################################
    ...
    def get_w_vmix(self,simul,frontal=False, thermal = True, **kwargs):
        ...
        buoy = tools.vinterp(-simul.g * toolsF.rho1_eos(T,S,z_r,simul.rho0)/simul.rho0,depths,z_r,z_w)
        AKv = tools.vinterp(self.load('AKv',ncfile,simul,coord=coord, depths=depths_w),depths,z_r,z_w,kmin=0) #*0.+4e-2
        
        buoy0 = tools.vinterp(-simul.g * toolsF.rho1_eos(T,S,z_r,simul.rho0)/simul.rho0,depths0,z_r,z_w)
        AKv0 = tools.vinterp(self.load('AKv',ncfile,simul,coord=coord, depths=depths_w),depths0,z_r,z_w,kmin=0) #*0.+4e-2
        
        buoy1 = tools.vinterp(-simul.g * toolsF.rho1_eos(T,S,z_r,simul.rho0)/simul.rho0,depths1,z_r,z_w)
