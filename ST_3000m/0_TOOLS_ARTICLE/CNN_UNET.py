...
class CNN_UNET_SURF(pl.LightningModule):
    ...
    def forward(self, z, y_filter,y):
        ...
        y_pred = torch.zeros((y.shape[0],y.shape[1],z.shape[2],z.shape[3])).to('cuda')
        if z.shape[2]%2==0:
            index_mid = int(z.shape[2]/2) 
            y_pred[:,:,index_mid-1:index_mid+1,index_mid-1:index_mid+1] = 0.25 # Initialisation of y_pred
        else :
            index_mid = int(z.shape[2]/2)
            y_pred[:,:,index_mid,index_mid]=1
        ...
        y_hat = y_hat.view(y_hat.shape[0],8,dx_pdf,dx_pdf)
...
class CNN_UNET_generic(pl.LightningModule):
    def __init__(self,kernel_size,padding,bias,p_dropout,nlayer0,nb_inputs):
        ...
        ### ADD convTrans for 100x100 ###
        #if nb_convTrans4 > 0:
        #self.convTrans4 = nn.ConvTranspose2d(8,8,kernel_size = 2,padding = 0,stride=2)
        #self.convTrans3_impair = nn.ConvTranspose2d(nlayer0*4,nlayer0*2,kernel_size = 2,padding = 0,stride=2,output_padding=1)
        #self.convTrans2_impair = nn.ConvTranspose2d(nlayer0*2,nlayer0,kernel_size = 2,padding = 0,stride=2,output_padding=1)
    ...
    def forward(self, z, y_filter,y):
        ...
        # y_pred : tensor source of particles
        y_pred = torch.zeros((y.shape[0],y.shape[1],z.shape[2],z.shape[3])).to(device)
        if z.shape[2]%2==0:
            index_mid = int(z.shape[2]/2)
            y_pred[:,:,index_mid-1:index_mid+1,index_mid-1:index_mid+1] = 0.25 # Initialisation of y_pred
        else :
        ...
        # if odd dx number, need to at output_padding 
        if y2.shape[3]%2==0: #pair
            y3 = self.convTrans3(y3)
        #else : 
        #    y3 = self.convTrans3_impair(y3)
        y3 = torch.cat((y2,y3),dim=1)
        y2 = self.conv_up2(y3)
        ...
        if y1.shape[3]%2==0:
            y2 = self.convTrans2(y2)
        #else :
        #    y2 = self.convTrans2_impair(y2)
        y2 = torch.cat((y1,y2),dim=1)
        y1 = self.conv_up1(y2)
        
        # If nb_dx != 100, need to add convTrans
        #if nb_convTrans4 > 0:
        #    for i in range(nb_convTrans4):
        #         y1 = self.convTrans4(y1)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss_filter_200m = Bhatta_loss(y_hat[:,-1,:,:], y_f[:,-1,:,:])
        loss_nofilter_200m = Bhatta_loss(y_hat[:,-1,:,:], y[:,-1,:,:])
        loss_200m_alpha = alpha1*Bhatta_loss(y_hat[:,-1,:,:], y_f[:,-1,:,:]) + alpha2*Bhatta_loss(y_hat[:,-1,:,:], y[:,-1,:,:])

        #self.log("loss_filter_validation", loss_filter, on_epoch=True, on_step = True)
        #self.log("loss_no_filter_validation", loss_no_filter, on_epoch=True, on_step = True)
        self.log("loss_filter_200m_validation", loss_filter_200m, on_epoch=True, on_step = True)
        self.log("loss_200m_validation", loss_nofilter_200m, on_epoch=True, on_step = True)
        self.log("loss_alpha_200m_validation", loss_200m_alpha, on_epoch=True, on_step = True)
...
class cnn_parallele(pl.LightningModule):
    ...
    def forward(self, z):
        ...
        out = out.view(out.shape[0],1,dx_pdf,dx_pdf)
